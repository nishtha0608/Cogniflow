      ...
      {STAGES.map((s, i) => (
        <div
          ...
          className={cn(
            ...
            i < idx ? 'bg-slate-500' : i === idx ? '' : 'bg-gray-200 dark:bg-slate-700',
          )}
...
function InsightsContent({ project }) {
  ...
  if (isLoading) return (
    <div className="flex items-center gap-2 text-gray-400 dark:text-slate-500 text-xs py-4">
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400 leading-relaxed mt-1">{data.narrative}</p>
          {data.predicted_contribution && (
            <p className="text-xs text-gray-400 dark:text-slate-500 mt-1 italic">{data.predicted_contribution}</p>
          )}
        </div>
      </div>
          ...
          <ol className="space-y-2">
            {data.next_actions.map((a, i) => (
              <li key={i} className="flex items-start gap-2.5">
                ...
                <span className="text-sm text-gray-700 dark:text-slate-300 leading-relaxed">{a}</span>
              </li>
            ))}
          </ol>
        </div>
            ...
            return (
              <div key={i} className="rounded-xl p-3 border border-rose-500/10 bg-rose-500/5">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{r.risk}</span>
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ color: sc, background: `${sc}18` }}>
                    {r.severity}
                  </span>
                </div>
                <p className="text-xs text-gray-400 dark:text-slate-500 leading-relaxed">{r.mitigation}</p>
              </div>
            );
          })}
        </div>
      ...
      {data.velocity_tip && (
        <div className="flex items-start gap-2.5 p-3 rounded-xl bg-violet-500/8 border border-violet-500/15">
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 leading-relaxed">{data.velocity_tip}</p>
        </div>
      )}
    </div>
  );
...
function ProjectCard({ project, onEdit, onDelete, onOpen, onOpenWorkspace }) {
  ...
  return (
    <motion.div
      ...
      layout
      className="relative rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-gray-200 dark:border-slate-800 overflow-hidden group hover:border-gray-200 dark:hover:border-slate-700 transition-all duration-300 cursor-pointer"
      onClick={onOpen}
    >
      {/* Top accent bar */}
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="relative flex items-center justify-center shrink-0 mt-0.5">
            ...
            <span className="absolute text-[9px] font-black text-gray-900 dark:text-white">{progress}%</span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3 className="text-sm font-bold text-gray-900 dark:text-white leading-snug line-clamp-2">{project.title}</h3>
          </div>
          ...
          >
            <button
              onClick={onEdit}
              className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-400 dark:text-slate-500 hover:text-gray-800 dark:hover:text-white transition-colors"
            >
              <Edit3 size={13} />
            </button>
            <button
              onClick={onDelete}
              className="p-1.5 rounded-lg hover:bg-rose-500/10 text-gray-400 dark:text-slate-500 hover:text-rose-400 transition-colors"
            >
              <Trash2 size={13} />
            </button>
          </div>
        ...
        {/* Abstract */}
        {project.abstract && (
          <p className="text-xs text-gray-400 dark:text-slate-500 line-clamp-2 leading-relaxed">{project.abstract}</p>
        )}

        {/* Stage pipeline */}
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-400 dark:text-slate-500">{stage?.name || 'Ideation'}</span>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {project.deadline && (
            <div className={cn(
              ...
              deadlineOverdue ? 'text-rose-400' : deadlineUrgent ? 'text-amber-400' : 'text-gray-400'
            )}>
              <Calendar size={11} />
              {deadlineOverdue
        ...
        {project.keywords?.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {project.keywords.slice(0, 4).map((kw, i) => (
              <span
                key={i}
                className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-slate-800 border border-gray-300/50 dark:border-slate-700 text-gray-500 dark:text-slate-400"
              >
                {kw}
              </span>
            ))}
            {project.keywords.length > 4 && (
              <span className="text-xs text-gray-400 dark:text-slate-500">+{project.keywords.length - 4}</span>
            )}
          </div>
        )}
...
function DetailSection({ title, icon: Icon, color, children }) {
  return (
    <div className="rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-gray-200 dark:border-slate-800 overflow-hidden">
      <div
        className="flex items-center gap-2.5 px-5 py-3.5 border-b border-gray-200 dark:border-slate-800"
...
function RecommendedPapers({ keywords }) {
  ...
  return (
    <div className="rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-gray-200 dark:border-slate-800 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70">
        <div className="flex items-center gap-2.5">
          <Sparkles size={14} className="text-violet-400" />
          <span className="text-xs font-semibold uppercase tracking-widest text-violet-400">Recommended Papers</span>
        </div>
        <Link to={createPageUrl('PaperSearch')} className="text-xs text-gray-400 dark:text-slate-500 hover:text-violet-400 flex items-center gap-1 transition-colors">
      ...
      {loading ? (
        ...
        <p className="text-xs text-gray-400 dark:text-slate-500 text-center py-6">No recommendations found</p>
      ) : (
        <div className="divide-y divide-white/5">
          {papers.map(paper => (
            <div key={paper.id} className="px-5 py-3.5 hover:bg-white/70 dark:hover:bg-slate-800/70 transition-colors">
              <p className="text-xs font-medium text-gray-800 dark:text-slate-100 leading-snug line-clamp-2 mb-1">{paper.title}</p>
              <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400 dark:text-slate-500">
                {paper.authors?.slice(0, 2).join(', ')}{paper.authors?.length > 2 ? ' et al.' : ''}
                {paper.year && <span className="text-gray-400 dark:text-slate-500">·</span>}
                {paper.year && <span>{paper.year}</span>}
                {paper.cited_by_count > 0 && <span className="text-gray-400 dark:text-slate-500">· {paper.cited_by_count.toLocaleString()} citations</span>}
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                {paper.doi && (
                  <button onClick={() => copyDoi(paper.doi)}
                    className="text-xs text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-white flex items-center gap-1 transition-colors">
...
function ProjectDetail({ project, onBack, onEdit, onDelete, onOpenWorkspace }) {
  ...
  return (
    <motion.div
      ...
      className="min-h-screen bg-violet-50 dark:bg-slate-950 p-6 md:p-8"
    >
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <button
            onClick={onBack}
            className="flex items-center gap-2 text-gray-400 dark:text-slate-500 hover:text-gray-900 dark:hover:text-white transition-colors text-sm font-medium group"
          >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-gray-900 transition-all shadow-lg hover:opacity-90"
              style={{
            ...
            <button
              onClick={onEdit}
              className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white text-xs font-semibold transition-colors"
            >
              <Edit3 size={13} />
              Edit
            </button>
        ...
        {/* Hero header */}
        <div
          className="relative rounded-2xl border border-gray-200 dark:border-slate-800 overflow-hidden"
            ...
            <div className="flex items-start gap-6 flex-wrap md:flex-nowrap">
              ...
              <div className="relative flex items-center justify-center shrink-0">
                ...
                <div className="absolute text-center">
                  <span className="block text-lg font-black text-gray-900 dark:text-white leading-none">{progress}%</span>
                  <span className="block text-[9px] text-gray-400 dark:text-slate-500 leading-none mt-0.5">done</span>
                </div>
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <h1 className="text-xl md:text-2xl font-bold text-gray-900 leading-snug mb-3">{project.title}</h1>
              ...
              <div className="flex flex-col gap-2 shrink-0">
                ...
                {project.target_journal && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-xs text-gray-500 dark:text-slate-400">
                    <FileText size={13} />
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 space-y-5">
            ...
            {project.abstract && (
              <DetailSection title="Abstract" icon={Info} color={fieldMeta.color}>
                <p className="text-sm text-gray-700 dark:text-slate-300 leading-relaxed">{project.abstract}</p>
              </DetailSection>
            )}
                ...
                <ol className="space-y-3">
                  {project.research_questions.map((q, i) => (
                    <li key={i} className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-gray-700 dark:text-slate-300 leading-relaxed">{q}</span>
                    </li>
                  ))}
                </ol>
              </DetailSection>
              ...
              <DetailSection title="Keywords" icon={Tag} color="#06b6d4">
                <div className="flex flex-wrap gap-2">
                  {project.keywords.map((kw, i) => (
                    <span
                      key={i}
                      className="text-sm px-3 py-1 rounded-full bg-gray-100 dark:bg-slate-800 border border-gray-300/50 dark:border-slate-700 text-gray-700 dark:text-slate-300"
                    >
                      {kw}
                    </span>
                  ))}
                    ...
                    <div key={s.id} className="flex items-center gap-3">
                      <div
                        className={cn(
                          ...
                          done ? 'bg-gray-200 dark:bg-slate-700 text-gray-500 dark:text-slate-400' : active ? 'text-white' : 'bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-400 dark:text-slate-500'
                        )}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span className={cn(
                            'text-sm font-medium',
                            done ? 'text-gray-400' : active ? 'text-white' : 'text-gray-400'
                          )}>
                            {s.name}
                          </span>
                          {active && (
                        ...
                        {active && (
                          <div className="mt-1 h-1 bg-gray-100 dark:bg-slate-800 rounded-full overflow-hidden">
                            <motion.div
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            {/* AI Insights */}
            <div className="rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-gray-200 dark:border-slate-800 overflow-hidden">
              <div className="flex items-center gap-2.5 px-5 py-3.5 border-b border-gray-200 dark:border-slate-800 bg-violet-500/8">
                <Sparkles size={14} className="text-violet-400" />
            ...
            {/* Quick Actions */}
            <div className="rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-gray-200 dark:border-slate-800 overflow-hidden">
              <div className="flex items-center gap-2.5 px-5 py-3.5 border-b border-gray-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70">
                ...
                {QUICK_ACTIONS.map(a => {
                  ...
                  return (
                    <button
                      ...
                      className="flex flex-col gap-1.5 p-3 rounded-xl border border-gray-200 dark:border-slate-700 hover:border-gray-200 dark:hover:border-slate-600 bg-white/70 dark:bg-slate-800/70 hover:bg-gray-100 dark:hover:bg-slate-800 transition-all group text-left"
                    >
                      <div
                      ...
                      <span className="text-xs font-semibold text-gray-900 dark:text-white">{a.label}</span>
                      <span className="text-xs text-gray-400 dark:text-slate-500 leading-tight">{a.description}</span>
                    </button>
                  );
                })}
              </div>
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-white/8 dark:border-slate-700 rounded-2xl overflow-hidden shadow-2xl"
        >
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={onClose}
                className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-400 dark:text-slate-500 hover:text-gray-900 dark:hover:text-white transition-colors"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Title *</label>
                <input
                  value={form.title}
                  onChange={e => setForm({ ...form, title: e.target.value })}
                  placeholder="Your research title…"
                  className="w-full bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-gray-900 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-500 focus:outline-none focus:border-violet-500/40 transition-colors"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Abstract</label>
                <textarea
                  value={form.abstract}
                  onChange={e => setForm({ ...form, abstract: e.target.value })}
                  placeholder="Brief description of your research…"
                  rows={3}
                  className="w-full bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-gray-900 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-500 resize-none focus:outline-none focus:border-violet-500/40 transition-colors"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Field *</label>
                  <select
                    value={form.field}
                    onChange={e => setForm({ ...form, field: e.target.value })}
                    className="w-full bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-gray-900 dark:text-slate-100 focus:outline-none focus:border-violet-500/40 transition-colors appearance-none cursor-pointer"
                  >
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Stage</label>
                  <select
                    value={form.stage}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Target Journal</label>
                  <input
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-widest">Deadline</label>
                  <input
                    type="date"
                    value={form.deadline}
                    onChange={e => setForm({ ...form, deadline: e.target.value })}
                    className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-violet-500/40 transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-widest">
                  Research Questions <span className="text-gray-400 normal-case font-normal">(one per line)</span>
                </label>
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 placeholder-slate-500 resize-none focus:outline-none focus:border-violet-500/40 transition-colors"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-widest">
                  Keywords <span className="text-gray-400 normal-case font-normal">(comma separated)</span>
                </label>
                <input
                  value={form.keywords}
                  onChange={e => setForm({ ...form, keywords: e.target.value })}
                  placeholder="machine learning, NLP, transformers…"
                  className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 placeholder-slate-500 focus:outline-none focus:border-violet-500/40 transition-colors"
                />
              </div>

              <div className="rounded-xl bg-gray-100/70 border border-gray-200 p-3 space-y-2">
                <span className="text-xs text-gray-400">Stage preview</span>
                <StagePipeline currentStage={form.stage} />
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500">{STAGES.find(s => s.id === form.stage)?.name}</span>
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Track, manage, and accelerate your research projects</p>
            </div>
          </div>
          <button
            ...
            {[
              ...
              return (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="rounded-xl bg-white/80 border border-gray-200 px-4 py-3 flex items-center gap-3"
                >
        ...
        {projects.length > 0 && (
          <div className="flex flex-wrap gap-2">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-xs text-gray-400 font-semibold uppercase tracking-widest pr-1">Stage</span>
        ...
        {!isLoading && projects.length === 0 && (
          ...
          >
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-900 mb-1">Start your research portfolio</h3>
